...
func TestWalletEarnEthereumVaultDetails(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Wallets.Earn.Ethereum.VaultDetails(context.TODO(), "vault_id")
	if err != nil {
		var apierr *privyclient.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
...
func TestWalletEarnEthereumVaultPositionWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Wallets.Earn.Ethereum.VaultPosition(
		context.TODO(),
		"wallet_id",
		privyclient.WalletEarnEthereumVaultPositionParams{
			VaultID:         "vault_id",
