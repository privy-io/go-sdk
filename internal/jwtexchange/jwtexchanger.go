...
type JwtExchanger interface {
	...
	ExchangeJwtForAuthorizationKey(jwt string) (string, error)
}
...
func ExchangeJwtsForKeys(exchanger JwtExchanger, jwts []string) ([]string, error) {
	keys := make([]string, 0, len(jwts))
	for i, jwt := range jwts {
		key, err := exchanger.ExchangeJwtForAuthorizationKey(jwt)
		if err != nil {
